...
export default function PublicLiveResultsPage() {
  ...
  const categories = useMemo(() => [...new Set((payload?.results ?? []).map((result) => result.category))].sort(), [payload]);
  const stageResults = useMemo(() => {
    const rows = (payload?.results ?? []).filter((result) => result.stage_id === stageId);
  ...
  return <main className="live-results">
    ...
    <section className="live-shell live-content">
      ...
      {view === "stage" && payload?.stages.length ? <select aria-label="Etapa" value={stageId} onChange={(event) => setStageId(event.target.value)} style={{width:"100%",padding:14,marginBottom:18,background:"#161a16",color:"#fff",border:"1px solid #474c44"}}>{payload.stages.map((stage) => <option value={stage.id} key={stage.id}>Stage {stage.stage_number} · {stage.name}</option>)}</select> : null}
