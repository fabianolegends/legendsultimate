...
function normalizeInput(input: RegistrationInput, forcedEventId?: string) {
  ...
  return {
    ...
    full_name: fullName,
    email,
    birth_date: cleanNullable(input.birth_date),
    gender: cleanNullable(input.gender),
    category: cleanNullable(input.category),
    modality: input.modality === "experience" ? "experience" : "gravel_race",
    country_code: cleanNullable(input.country_code)?.toUpperCase() ?? null,
    city: cleanNullable(input.city),
    status: ["pending", "confirmed", "waitlist", "cancelled"].includes(String(input.status))
      ? input.status
      : "confirmed",
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let query = supabase
      .from("registrations")
      .select("id, event_id, athlete_id, registration_code, bib_number, full_name, email, birth_date, gender, category, modality, country_code, city, status, claimed_at, created_at, updated_at")
      .order("full_name", { ascending: true });
    if (eventId) query = query.eq("event_id", eventId);
    const { data: registrations, error } = await query;
    if (error) {
      if (error.code === "42P01") {
        return NextResponse.json({ module_ready: false, events: events ?? [], registrations: [], summary: null });
      }
      throw error;
    }
  ...
  try {
    ...
    if (body.action === "import") {
      ...
      const { data, error } = await supabase
        ...
        .select("id, event_id, athlete_id, full_name, email, category, modality, status");
      if (error) throw error;
