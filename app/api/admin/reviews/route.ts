...
export async function GET(request:NextRequest){
  ...
  try{
    ...
    const activityIds=[...new Set((validations??[]).map(v=>v.activity_id))];
    const stageIds=[...new Set((validations??[]).map(v=>v.stage_id))];
    const {data:activities,error:activityError}=activityIds.length?await supabase.from("activities").select("id, athlete_id, stage_id, source, source_activity_id, name, started_at, distance_km, elevation_m, moving_time_s, gps_points, created_at").in("id",activityIds):{data:[],error:null};
    if(activityError)throw activityError;
    const athleteIds=[...new Set((activities??[]).map(a=>a.athlete_id))];
    const {data:athletes,error:athleteError}=athleteIds.length?await supabase.from("athletes").select("id, full_name, strava_athlete_id, category, country_code").in("id",athleteIds):{data:[],error:null};
    if(athleteError)throw athleteError;
    const {data:stages,error:stageError}=stageIds.length?await supabase.from("stages").select("id, name, route_label, stage_date, distance_km, elevation_m").in("id",stageIds):{data:[],error:null};
    if(stageError)throw stageError;
    const {data:routes,error:routeError}=stageIds.length?await supabase.from("route_versions").select("id, stage_id, version, file_name, route_points, is_active").in("stage_id",stageIds).eq("is_active",true):{data:[],error:null};
    if(routeError)throw routeError;
    const {data:checkpoints,error:checkpointError}=stageIds.length?await supabase.from("checkpoints").select("id, stage_id, sequence, label, latitude, longitude, radius_m").in("stage_id",stageIds).order("sequence",{ascending:true}):{data:[],error:null};
    ...
    const items=(validations??[]).map(v=>{const activity:any=activityMap.get(v.activity_id);const athlete=activity?athleteMap.get(activity.athlete_id):null;const stage=stageMap.get(v.stage_id);const route:any=routeMap.get(v.stage_id);const stageCheckpoints=(checkpoints??[]).filter(c=>c.stage_id===v.stage_id).map(c=>({...c,hit:false,nearest_distance_m:0}));return{...v,activity:activity?{...activity,gps_points:sample(activity.gps_points)}:null,athlete,stage,route:route?{...route,route_points:sample(route.route_points)}:null,checkpoints:stageCheckpoints};});
...
export async function PATCH(request:NextRequest){
  ...
  try{
    ...
    const validationId=String(body.validationId??"");const decision=body.decision;const note=String(body.note??"").trim();
    ...
    const supabase=createSupabaseAdmin();
    const {data:current,error:currentError}=await supabase.from("validation_results").select("id, notes").eq("id",validationId).single();
    ...
    const {error}=await supabase.from("validation_results").update({status:decision,notes,validated_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",validationId);
    if(error)throw error;
