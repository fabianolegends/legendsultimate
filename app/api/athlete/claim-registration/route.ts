...
export async function POST(request: NextRequest) {
  try {
    ...
    if (registration.status !== "confirmed") {
      ...
      return NextResponse.json({ error: `Esta inscrição está ${statusText} e ainda não libera o Passport.` }, { status: 403 });
    }
    ...
    const { data: linked, error: linkError } = await supabase
      ...
      .select("id, event_id, registration_code, bib_number, full_name, email, category, modality, country_code, city, status, claimed_at")
      .single();
    if (linkError) throw linkError;

    return NextResponse.json({ linked: true, registration: linked });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Falha ao vincular a inscrição." }, { status: 500 });
  }
}
