  ...
  return (
    <main className="registrationPage">
      <style>{`
        ...
        .lotGrid{display:grid;grid-template-columns:repeat(3,1fr);border:1px solid var(--line)}.lot{padding:28px;border-right:1px solid var(--line)}.lot:last-child{border:0}.lot span{color:var(--copper);font:600 13px 'Barlow Condensed';letter-spacing:.13em;text-transform:uppercase}.lot strong{display:block;font:700 48px 'Barlow Condensed';margin:22px 0 8px}.lot p{margin:0;color:#9ba097}
        ...
        @media(max-width:850px){.heroGrid,.columns,.modeGrid{grid-template-columns:1fr}.lotGrid{grid-template-columns:1fr}.lot{border-right:0;border-bottom:1px solid var(--line)}.lot:last-child{border-bottom:0}.docs{grid-template-columns:1fr}.premium{grid-template-columns:1fr}.regHero{padding-top:60px}.registrationPage .shell{width:min(100% - 30px,1180px)}}
      `}</style>
      ...
      <section className="section">
        <div className="shell"><p className="eyebrow">Valores</p><h2>Três lotes.<br /><em>100 lugares.</em></h2><div className="lotGrid">{launchConfig.lots.map(lot=><article className="lot" key={lot.name}><span>{lot.name}</span><strong>{lot.price}</strong><p>{lot.period}</p></article>)}</div></div>
      </section>
