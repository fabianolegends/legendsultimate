  ...
  return (
    <main className="registrationPage">
      <style>{`
        ...
        .regHero h1{margin:0;max-width:850px;font:700 clamp(40.6px,5.6vw,78.4px) 'Barlow Condensed';line-height:.86;text-transform:uppercase}.regHero h1 em{font-style:normal;color:var(--copper)}
      ...
      <section className="journeySection" id="jornadas">
        <div className="shell">
          ...
          <div className="journeyGrid">
            {Object.values(launchConfig.journeys).map((option) => <a className={`journeyCard${option.id === format ? " active" : ""}`} href={`/inscricoes?formato=${option.id}#jornadas`} key={option.id}><div><span className="tag">{option.id === format ? "Formato selecionado" : "Selecionar formato"}</span><h3><span>Legends</span><em className="journeyScript">{option.id === "short" ? "Short" : "Ultimate"}</em></h3><p><span className="journeyDate">{option.dateLabel}</span><br />{option.cities}</p></div><div className="journeyStats"><strong>{option.lots[0].price}</strong><span>Lote 01 · {option.spots} vagas</span><span>{option.days} dias · {option.stageNumbers.length} etapas</span></div><span className="journeyAction"><span>{option.distance} · {option.ascent}</span><span>Ver informações e lotes →</span></span></a>)}
          </div>
