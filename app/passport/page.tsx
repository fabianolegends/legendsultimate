...
function haversine(a: [number, number], b: [number, number]) {
  ...
  const value = Math.sin(dLat / 2) ** 2 + Math.cos((a[0] * Math.PI) / 180) * Math.cos((b[0] * Math.PI) / 180) * Math.sin(dLon / 2) ** 2;
...
function parseGpx(text: string) {
  ...
  if (points.length < 2) throw new Error("O GPX não contém pontos suficientes.");

  let distanceKm = 0;
  let elevationM = 0;
  for (let index = 1; index < points.length; index += 1) {
    distanceKm += haversine([points[index - 1].lat, points[index - 1].lon], [points[index].lat, points[index].lon]);
    const gain = points[index].ele - points[index - 1].ele;
    if (gain > 0) elevationM += gain;
  }

  const start = points.find((point) => point.time)?.time;
  const end = [...points].reverse().find((point) => point.time)?.time;
...
export default function Passport() {
  ...
  const currentResult = results[stage.id];
  const total = useMemo(() => Object.values(results).reduce((sum, result) => sum + result.distanceKm, 0), [results]);
  const totalPoints = useMemo(() => Object.values(results).reduce((sum, result) => sum + result.points, 0), [results]);
    ...
    <main className="passport">
      ...
      <div className="shell">
        <section className="hero">
          ...
          <div className="summary"><div><strong>{Object.keys(results).length}/4</strong><span>etapas enviadas</span></div><div><strong>{total.toFixed(0)} km</strong><span>registrados</span></div><div><strong>{totalPoints}</strong><span>pontos</span></div></div>
        </section>
        <section className="grid">
          ...
          <div className="panel">
            ...
            {currentResult && <div className="result"><div><strong>{currentResult.distanceKm.toFixed(1)} km</strong><span>distância</span></div><div><strong>{Math.round(currentResult.elevationM)} m+</strong><span>elevação</span></div><div><strong>{Math.round(currentResult.durationMin)} min</strong><span>tempo</span></div><div><strong>{currentResult.avgSpeed.toFixed(1)} km/h</strong><span>média</span></div><div className="status">{currentResult.status} · {currentResult.points} pontos</div><div className="extra">Fonte: {currentResult.source}{currentResult.avgHeartRate != null ? ` · FC média ${Math.round(currentResult.avgHeartRate)} bpm` : ""}{currentResult.avgWatts != null ? ` · Potência média ${Math.round(currentResult.avgWatts)} W` : ""}</div></div>}
