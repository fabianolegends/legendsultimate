...
export default function AthleteAccessPage() {
  const [state, setState] = useState<RideWithGpsState | null>(null);

  useEffect(() => {
    fetch("/api/ridewithgps/trips", { cache: "no-store" })
      .then((response) => response.json())
      .then((payload) => setState(payload))
      .catch(() => setState({ configured: false, connected: false, athlete: null }));
  }, []);

  const name = state?.athlete?.name?.trim() ?? "";
  ...
  return (
    <main className="athlete-access">
      <style>{`
        ...
        .athlete-box{width:min(520px,100%);border:1px solid rgba(212,123,45,.48);background:rgba(15,18,15,.94);padding:38px;box-sizing:border-box}.athlete-box img{width:180px;margin-bottom:34px}.kicker{color:#d47b2d;letter-spacing:.2em;text-transform:uppercase;font-size:12px;font-weight:900}.athlete-box h1{font-size:48px;line-height:.95;text-transform:uppercase;margin:15px 0 20px}.athlete-box p{color:#b9bdb7;line-height:1.7}.steps{display:grid;gap:10px;margin:26px 0}.step{border-top:1px solid #363b34;padding-top:12px;color:#d9d4cb}.step strong{color:#d47b2d;margin-right:10px}.ride-button,.passport-button{display:block;text-align:center;padding:17px;color:white;text-decoration:none;font-weight:900;text-transform:uppercase;margin-top:20px}.ride-button{background:#45755f}.passport-button{background:#e86619}.secondary{display:block;text-align:center;margin-top:18px;color:#b8bdb5;font-size:13px}.athlete-profile{display:flex;gap:14px;align-items:center;border:1px solid #41473e;padding:14px;margin:24px 0}.athlete-profile strong,.athlete-profile span{display:block}.athlete-profile span{font-size:12px;color:#9da39a;margin-top:4px}
      ...
      <section className="athlete-box">
        ...
        <p>Use sua conta Ride with GPS para entrar. A Legends acessará apenas os dados necessários para localizar e homologar suas atividades.</p>
        ...
        {!state ? <p>Verificando sua conexão...</p> : state.connected ? (
          <>
            <div className="athlete-profile">
              <div><strong>{name || "Atleta conectado"}</strong><span>Conta Ride with GPS conectada</span></div>
            </div>
            <a className="passport-button" href="/passport/atleta">Entrar no Passport</a>
          </>
        ) : state.configured ? (
          <a className="ride-button" href="/api/ridewithgps/connect">Conectar com Ride with GPS</a>
        ) : (
          <p style={{ color: "#efb078" }}>A integração com Ride with GPS ainda não está configurada.</p>
        )}
