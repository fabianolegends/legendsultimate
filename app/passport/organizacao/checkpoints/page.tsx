...
type StoredCheckpoint = EditorCheckpoint & { id?: string; checkpoint_kind?: string; is_timing_point?: boolean };
type Segment = {
  id: string;
  name: string;
...
type Payload = {
  ...
  stage: Stage;
  route: { id: string; version: number; file_name: string; route_points: Array<[number, number, number?]> };
  checkpoints: StoredCheckpoint[];
  segments: Segment[];
};

function pointAtProgress(points: EditorPoint[], progress: number) {
  const index = Math.round((Math.min(100, Math.max(0, progress)) / 100) * Math.max(0, points.length - 1));
  return points[index] ?? points[0] ?? [-29.5, -50.8];
}

function generateUniform(points: EditorPoint[], intermediateCount: number): StoredCheckpoint[] {
...
export default function CheckpointManagerPage() {
  ...
  const [segmentType, setSegmentType] = useState<"climb" | "sprint" | "custom">("climb");
  ...
  const routePoints = useMemo<EditorPoint[]>(
    () => (payload?.route.route_points ?? []).map((point) => [Number(point[0]), Number(point[1])]),
    [payload],
  );
  ...
  async function loadCheckpointData(currentStageId: string) {
    setLoading(true);
    setStatus("");
    const response = await fetch(`/api/admin/checkpoints?stageId=${encodeURIComponent(currentStageId)}`, { cache: "no-store" });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error ?? "Falha ao carregar checkpoints.");
    const nextPayload = data as Payload;
    const points = (nextPayload.route.route_points ?? []).map((point) => [Number(point[0]), Number(point[1])] as EditorPoint);
    const saved = nextPayload.checkpoints?.length ? nextPayload.checkpoints : generateUniform(points, 5);
    setPayload(nextPayload);
    setCheckpoints(saved);
    setIntermediateCount(Math.max(1, saved.length - 2));
    setSelectedIndex(saved.length > 2 ? 1 : 0);
    const firstControl = saved.find((checkpoint, index) => index > 0 && index < saved.length - 1);
    setSegmentStart(firstControl?.id ?? saved[0]?.id ?? "");
    setSegmentFinish(saved[saved.length - 2]?.id ?? saved[saved.length - 1]?.id ?? "");
    setLoading(false);
  }
  ...
  useEffect(() => {
    if (!stageId) return;
    loadCheckpointData(stageId).catch((error) => { setStatus(error.message); setLoading(false); });
  }, [stageId]);
  ...
  function updateProgress(index: number, progress: number) {
    ...
    setCheckpoints((current) => current.map((checkpoint, checkpointIndex) => checkpointIndex === index
      ? { ...checkpoint, route_progress: Number(normalized.toFixed(2)), latitude: point[0], longitude: point[1] }
      : checkpoint));
  }

  function updateCheckpoint(index: number, patch: Partial<StoredCheckpoint>) {
    setCheckpoints((current) => current.map((checkpoint, checkpointIndex) => checkpointIndex === index ? { ...checkpoint, ...patch } : checkpoint));
  }
  ...
  function regenerate() {
    ...
    setStatus("Distribuição uniforme criada. Revise os pontos no mapa e clique em salvar.");
  }
  ...
  async function saveCheckpoints() {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error ?? "Falha ao salvar checkpoints.");
      setStatus(`${data.checkpoints.length} pontos de controle salvos.`);
      await loadCheckpointData(stageId);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "Falha ao salvar checkpoints.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function createSegment(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/admin/checkpoints", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ stageId, name: segmentName, segmentType, startCheckpointId: segmentStart, finishCheckpointId: segmentFinish }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error ?? "Falha ao criar segmento.");
      setSegmentName("");
      setStatus("Segmento cronometrado criado.");
      await loadCheckpointData(stageId);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "Falha ao criar segmento.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function deleteSegment(segmentId: string) {
    ...
    const response = await fetch(`/api/admin/checkpoints?segmentId=${encodeURIComponent(segmentId)}`, { method: "DELETE" });
    const data = await response.json();
    if (!response.ok) { setStatus(data.error ?? "Falha ao excluir segmento."); return; }
    setStatus("Segmento excluído.");
    await loadCheckpointData(stageId);
  }

  const checkpointById = new Map(checkpoints.filter((checkpoint) => checkpoint.id).map((checkpoint) => [checkpoint.id, checkpoint]));
  ...
  return (
    <main className="checkpoint-page">
      ...
      <div className="checkpoint-shell">
        ...
        <p className="intro">Defina cinco pontos intermediários por etapa, ajuste cada posição sobre o GPX oficial e transforme pares de checkpoints em subidas, sprints ou trechos especiais.</p>
        <select className="stage-select" value={stageId} onChange={(event) => setStageId(event.target.value)}>
          {stages.map((stage) => <option key={stage.id} value={stage.id}>{stage.name} · {formatStageDate(stage.stage_date)}</option>)}
        </select>

        {payload?.migration_required ? <div className="migration"><strong>Módulo aguardando banco de dados.</strong><br />{payload.module_message}</div> : null}

        <section className="checkpoint-grid">
          <div className="map-card">
            {loading ? <p>Carregando rota...</p> : routePoints.length > 1 ? (
              <CheckpointEditorMap routePoints={routePoints} checkpoints={checkpoints} selectedIndex={selectedIndex} onSelect={setSelectedIndex} onMove={updateProgress} />
            ) : <p className="empty">A etapa precisa ter uma rota GPX ativa.</p>}
            <p className="hint">Selecione um checkpoint na lista ou no mapa. Depois clique sobre a linha laranja para reposicioná-lo no ponto oficial mais próximo.</p>
          </div>

          <aside className="editor-card">
            <div className="editor-top">
              <label><span>Checkpoints intermediários</span><input type="number" min={1} max={12} value={intermediateCount} onChange={(event) => setIntermediateCount(Number(event.target.value))} /></label>
              <button className="secondary" type="button" onClick={regenerate}>DISTRIBUIR</button>
            </div>
            <div className="checkpoint-list">
              {checkpoints.map((checkpoint, index) => (
                <div className={`checkpoint-row ${selectedIndex === index ? "active" : ""}`} key={`${index}-${checkpoint.route_progress}`}>
                  <div className="checkpoint-row-head"><button type="button" onClick={() => setSelectedIndex(index)}>{index === 0 ? "LARGADA" : index === checkpoints.length - 1 ? "CHEGADA" : `CP ${index}`}</button><span>{checkpoint.route_progress.toFixed(2)}%</span></div>
                  <input value={checkpoint.label} disabled={index === 0 || index === checkpoints.length - 1} onChange={(event) => updateCheckpoint(index, { label: event.target.value })} />
                  <input className="range" type="range" min={0} max={100} step={0.05} disabled={index === 0 || index === checkpoints.length - 1} value={checkpoint.route_progress} onChange={(event) => updateProgress(index, Number(event.target.value))} />
                  <div className="row-fields"><span>Raio de detecção</span><input type="number" min={30} max={500} value={checkpoint.radius_m} onChange={(event) => updateCheckpoint(index, { radius_m: Number(event.target.value) })} /></div>
                </div>
              ))}
            </div>
            <button className="primary" type="button" disabled={saving || !payload?.module_ready} onClick={saveCheckpoints} style={{ width: "100%", marginTop: 16 }}>{saving ? "SALVANDO..." : "SALVAR CHECKPOINTS"}</button>
        ...
        <section className="segment-card">
          ...
          <form className="segment-form" onSubmit={createSegment}>
            <label><span>Nome</span><input value={segmentName} onChange={(event) => setSegmentName(event.target.value)} placeholder="Ex.: Subida do Morro" /></label>
            <label><span>Tipo</span><select value={segmentType} onChange={(event) => setSegmentType(event.target.value as typeof segmentType)}><option value="climb">Subida</option><option value="sprint">Sprint</option><option value="custom">Especial</option></select></label>
            <label><span>Início</span><select value={segmentStart} onChange={(event) => setSegmentStart(event.target.value)}><option value="">Selecione</option>{checkpoints.filter((checkpoint) => checkpoint.id).map((checkpoint) => <option key={checkpoint.id} value={checkpoint.id}>{checkpoint.label}</option>)}</select></label>
            <label><span>Fim</span><select value={segmentFinish} onChange={(event) => setSegmentFinish(event.target.value)}><option value="">Selecione</option>{checkpoints.filter((checkpoint) => checkpoint.id).map((checkpoint) => <option key={checkpoint.id} value={checkpoint.id}>{checkpoint.label}</option>)}</select></label>
            <button className="primary" disabled={saving || !payload?.module_ready}>CRIAR</button>
          </form>
          <div className="segments">
            {payload?.segments?.length ? payload.segments.map((segment) => {
              const start = checkpointById.get(segment.start_checkpoint_id);
              const finish = checkpointById.get(segment.finish_checkpoint_id);
              return <div className="segment-item" key={segment.id}><div><strong>{segment.name}</strong><span>{segment.segment_type === "climb" ? "Subida" : segment.segment_type === "sprint" ? "Sprint" : "Especial"} · {start?.label ?? "Início"} → {finish?.label ?? "Fim"}</span></div><button className="danger" type="button" onClick={() => void deleteSegment(segment.id)}>EXCLUIR</button></div>;
            }) : <p className="empty">Nenhum segmento cronometrado criado nesta etapa.</p>}
          </div>
        </section>
      </div>
    </main>
