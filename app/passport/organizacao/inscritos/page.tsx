...
export default function RegistrationsPage() {
  ...
  `}</style><div className="shell">
    ...
    {!moduleReady?<div className="not-ready">Execute a migration 006_registrations_eligibility.sql no Supabase.</div>:null}{moduleReady&&!windfitReady?<div className="not-ready">Execute a migration 007_windfit_source.sql para ativar pagamento e sincronização Windfit.</div>:null}
    <section className="metrics">{[[summary?.total,"registros"],[summary?.eligible,"elegíveis"],[summary?.paid,"pagos"],[summary?.linked,"Strava vinculado"],[summary?.payment_pending,"pagamento pendente"],[summary?.refunded,"reembolsados"]].map(([value,label])=><div className="metric" key={String(label)}><strong>{value??"—"}</strong><span>{label}</span></div>)}</section>{message?<p className="message">{message}</p>:null}
