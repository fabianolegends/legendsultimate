...
type Registration = {
  ...
  payment_provider?: string | null; payment_amount_cents?: number | null; payment_checkout_id?: string | null;
  payment_checkout_url?: string | null; payment_checkout_status?: string | null; payment_expires_at?: string | null;
  payment_confirmed_at?: string | null; payment_refunded_at?: string | null; last_payment_event_at?: string | null;
  created_at: string; imported_at: string | null; last_synced_at: string | null; athlete?: { ride_with_gps_user_id?: number } | null;
};
...
type BibSequence = { id?: string; category: string; start_number: number; next_number?: number; padding: number };
type LinkAudit = {
  id: string; registration_id: string; ride_with_gps_user_id: number | null;
  action: "linked" | "unlinked" | "transferred_in" | "transferred_out" | "migration_unlinked";
  actor_type: "athlete" | "admin" | "system"; reason: string | null; created_at: string;
};
type FormState = {
...
function formatMoney(value?: number | null) { return typeof value === "number" ? new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(value / 100) : null; }
function paymentLabel(value: string) {
  return value === "paid" ? "Pago" : value === "pending" ? "Pendente" : value === "refunded" ? "Reembolsado"
    : value === "cancelled" ? "Cancelado" : value === "failed" ? "Falhou" : value === "chargeback" ? "Chargeback"
      : value === "risk_analysis" ? "Em análise" : "Cortesia";
}
...
function paymentOrigin(item: Registration) {
  if (item.payment_provider === "asaas") return { label: "Asaas", className: "asaas" };
  if (item.source === "windfit") return { label: "Windfit", className: "windfit" };
...
export default function RegistrationsPage() {
  ...
  const [moduleReady, setModuleReady] = useState(true); const [windfitReady, setWindfitReady] = useState(true); const [detailsReady,setDetailsReady]=useState(true); const [paymentReady,setPaymentReady]=useState(true); const [numberingReady,setNumberingReady]=useState(true);
  const [identityReady,setIdentityReady]=useState(true); const [linkAudit,setLinkAudit]=useState<LinkAudit[]>([]); const [transferFromId,setTransferFromId]=useState("");
  const [drawer,setDrawer]=useState<"form"|"import"|"numbering"|null>(null); const [sequenceRows,setSequenceRows]=useState<BibSequence[]>([]);
  ...
  async function load(preferredEvent?: string) {
    ...
    const payload = await response.json(); if (!response.ok) throw new Error(payload.error ?? "Falha ao carregar inscritos.");
    setModuleReady(payload.module_ready !== false); setWindfitReady(payload.windfit_ready !== false); setDetailsReady(payload.details_ready !== false); setPaymentReady(payload.payment_ready !== false); setEvents(payload.events ?? []);
    const nextEvent = selected || payload.events?.[0]?.id || ""; if (!selected && nextEvent) setEventId(nextEvent);
    setItems(payload.registrations ?? []); setSummary(payload.summary ?? null); setNumberingReady(payload.numbering_ready !== false); setIdentityReady(payload.identity_ready !== false); setLinkAudit(payload.link_audit ?? []); setSequenceRows(payload.sequences ?? []); setForm((current) => ({ ...current, event_id: current.event_id || nextEvent })); if (payload.message) setMessage(payload.message);
  }
  ...
  const linkedCandidates = useMemo(() => items.filter((item) => item.id !== editingId && item.athlete_id && item.status !== "cancelled"), [items, editingId]);
  const editingAudit = useMemo(() => linkAudit.filter((item) => item.registration_id === editingId), [linkAudit, editingId]);
  ...
  function openNumbering() {
    ...
    const available = [...new Set([...categories, ...items.map((item) => item.category).filter(Boolean) as string[]])];
    setSequenceRows(available.map((category,index) => configured.get(category) ?? { category, start_number: index * 100 + 1, padding: 3 }));
    setDrawer("numbering");
  }
  function closeDrawer() { setEditingId(""); setTransferFromId(""); setForm({ ...emptyForm, event_id: eventId }); setDrawer(null); }
  ...
  async function saveSequences(event: FormEvent) {
    event.preventDefault(); setSaving(true); setMessage("Salvando sequências por categoria...");
    try {
      const response = await fetch("/api/admin/registrations", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ action:"configure_bib_sequences", eventId, sequences:sequenceRows }) });
      const payload = await response.json(); if(!response.ok) throw new Error(payload.error??"Falha ao configurar a numeração.");
  ...
  async function transferRegistrationIdentity() {
    if(!editingRegistration || editingRegistration.athlete_id || !transferFromId) return;
    const source=items.find((item)=>item.id===transferFromId);
    if(!source || !window.confirm(`Transferir a conta Ride with GPS vinculada a ${source.full_name} para ${editingRegistration.full_name}?\n\nEssa ação ficará registrada no histórico.`)) return;
    setSaving(true); setMessage("Transferindo identidade Ride with GPS...");
    try {
      const response=await fetch("/api/admin/registrations",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"transfer_registration_identity",fromId:source.id,toId:editingRegistration.id,reason:`Correção administrativa: ${source.full_name} → ${editingRegistration.full_name}`})});
      const payload=await response.json(); if(!response.ok) throw new Error(payload.error??"Falha ao transferir o vínculo.");
      setMessage(`A conta Ride with GPS foi transferida para ${editingRegistration.full_name}.`); closeDrawer(); await load(eventId);
    } catch(error) { setMessage(error instanceof Error?error.message:"Falha ao transferir o vínculo."); } finally { setSaving(false); }
  }
  ...
  `}</style><div className="shell">
    <section className="head"><div><p className="kicker">Inscrições → Legends Core</p><h1>Inscritos</h1></div><div><p>O Legends Engine reúne inscrições do Asaas, Windfit, convites e correções manuais. A elegibilidade esportiva só é liberada após pagamento confirmado ou cortesia autorizada.</p><div className="sync">Última sincronização externa: <strong>{formatDateTime(summary?.last_sync)}</strong></div></div></section>
    ...
    {moduleReady&&!paymentReady?<div className="not-ready">Execute a migration 022_asaas_checkout.sql para ativar checkout e confirmação automática do Asaas.</div>:null}
    {moduleReady&&!numberingReady?<div className="not-ready">Execute a migration 014_category_bib_sequences.sql para ativar a numeração automática por categoria.</div>:null}
    {moduleReady&&!identityReady?<div className="not-ready">Execute a migration 016_athlete_identity_integrity.sql para ativar a proteção de identidade Ride with GPS.</div>:null}
    ...
    <section className="actions-bar"><span className="records-count">{filtered.length} de {items.length} inscrições exibidas</span><div className="actions-main"><button className="action-button" onClick={openNumbering}>NUMERAÇÃO POR CATEGORIA</button><button className="action-button" onClick={()=>setDrawer("import")}>IMPORTAR WINDFIT</button><button className="action-button primary-action" onClick={startNew}>NOVA INSCRIÇÃO</button></div></section>
    <section className="workspace">
      {drawer?<><button type="button" className="drawer-backdrop" aria-label="Fechar painel" onClick={closeDrawer}/><form noValidate={drawer==="numbering"} className={`panel drawer-panel ${drawer==="import"?"import-only":drawer==="numbering"?"numbering":""}`} onSubmit={drawer==="numbering"?saveSequences:save}><div className="drawer-head"><h2>{drawer==="import"?"Importar Windfit":drawer==="numbering"?"Numeração por categoria":editingId?"Editar registro":"Nova inscrição"}</h2><button type="button" className="drawer-close" aria-label="Fechar" onClick={closeDrawer}>×</button></div><div className="manual-fields"><p className="panel-intro">Use o cadastro manual somente para cortesia, convidado ou correção administrativa. As inscrições comerciais entram pelo checkout Asaas ou pela importação Windfit, conforme a configuração do evento.</p><div className="form-grid">
      ...
      </div><button className="primary" disabled={saving}>{saving?"SALVANDO...":editingId?"ATUALIZAR REGISTRO":"CRIAR EXCEÇÃO MANUAL"}</button>{editingRegistration?.athlete_id?<button className="secondary" type="button" disabled={saving} onClick={unlinkRegistration}>DESVINCULAR RIDE WITH GPS</button>:null}{editingRegistration&&!editingRegistration.athlete_id&&linkedCandidates.length?<div className="identity-box"><h3>Corrigir vínculo</h3><p className="panel-intro">Transfira para esta inscrição uma conta que foi vinculada ao participante errado neste mesmo evento.</p><select value={transferFromId} onChange={(event)=>setTransferFromId(event.target.value)}><option value="">Selecione a inscrição de origem</option>{linkedCandidates.map((item)=><option key={item.id} value={item.id}>{item.full_name} · Ride {item.athlete?.ride_with_gps_user_id??""}</option>)}</select><button className="secondary" type="button" disabled={saving||!transferFromId} onClick={transferRegistrationIdentity}>TRANSFERIR CONTA PARA ESTA INSCRIÇÃO</button></div>:null}{editingId&&identityReady?<div className="identity-history"><h3>Histórico do vínculo</h3>{editingAudit.length?editingAudit.map((audit)=><div className="audit-item" key={audit.id}><strong>{audit.action==="linked"?"Conta vinculada":audit.action==="unlinked"?"Conta desvinculada":audit.action==="transferred_in"?"Conta recebida por transferência":audit.action==="transferred_out"?"Conta transferida":"Duplicidade antiga corrigida"}</strong><span>{formatDateTime(audit.created_at)} · {audit.actor_type}</span>{audit.reason?<span>{audit.reason}</span>:null}</div>):<p className="panel-intro">Nenhuma alteração de vínculo registrada.</p>}</div>:null}{editingId?<button className="secondary" type="button" onClick={closeDrawer}>CANCELAR EDIÇÃO</button>:null}</div>
      ...
      <section className="panel light"><div className="filters"><input placeholder="Buscar nome, e-mail, número, código ou ID externo" value={search} onChange={(e)=>setSearch(e.target.value)}/><select value={statusFilter} onChange={(e)=>setStatusFilter(e.target.value)}><option value="all">Todos os status</option><option value="confirmed">Confirmados</option><option value="pending">Pendentes</option><option value="waitlist">Lista de espera</option><option value="cancelled">Cancelados</option></select><select value={paymentFilter} onChange={(e)=>setPaymentFilter(e.target.value)}><option value="all">Todos os pagamentos</option><option value="paid">Pagos</option><option value="pending">Pendentes</option><option value="risk_analysis">Em análise</option><option value="refunded">Reembolsados</option><option value="cancelled">Cancelados</option><option value="failed">Falharam</option><option value="chargeback">Chargeback</option><option value="courtesy">Cortesias</option></select></div>
      {filtered.length?<div className="table-wrap"><table className="table"><thead><tr><th>Nº</th><th>Atleta</th><th>Categoria</th><th>Pagamento</th><th>Origem</th><th>Código</th><th>Vínculo</th><th>Status</th><th></th></tr></thead><tbody>{filtered.map((item)=>{const origin=paymentOrigin(item);return <tr key={item.id}><td>{item.bib_number??"—"}</td><td><strong>{item.full_name}</strong><span>{item.email}</span>{item.phone?<span>{item.phone}</span>:null}{item.location?<span>{item.location}</span>:null}{item.external_registration_id?<span>ID externo: {item.external_registration_id}</span>:null}</td><td>{item.category??"—"}<span>{item.modality==="experience"?"Experience":"Gravel Race"}</span></td><td><span className={item.payment_status}>{paymentLabel(item.payment_status)}</span>{formatMoney(item.payment_amount_cents)?<span>{formatMoney(item.payment_amount_cents)}</span>:null}{item.payment_confirmed_at?<span>Confirmado: {formatDateTime(item.payment_confirmed_at)}</span>:<span>Inscrição: {item.registered_at?formatDateTime(item.registered_at):formatDateTime(item.created_at)}</span>}{item.payment_checkout_url&&item.payment_status==="pending"?<a href={item.payment_checkout_url} target="_blank" rel="noreferrer">ABRIR CHECKOUT ↗</a>:null}</td><td><span className={`source ${origin.className}`}>{origin.label}</span><span>{item.payment_checkout_status??formatDateTime(item.last_synced_at)}</span></td><td className="code"><button type="button" className="copy-code" title="Copiar código de vínculo" onClick={()=>copyCode(item.registration_code)}>{item.registration_code}<small>{copiedCode===item.registration_code?"COPIADO ✓":"COPIAR"}</small></button></td><td><span className={item.athlete_id?"linked":"unlinked"}>{item.athlete_id?`✓ Ride with GPS ${item.athlete?.ride_with_gps_user_id??""}`:"Aguardando vínculo"}</span></td><td><span className={`status ${item.status}`}>{statusLabel(item.status)}</span></td><td><button className="edit" onClick={()=>edit(item)}>EDITAR</button></td></tr>})}</tbody></table></div>:<div className="empty-state"><div><strong>Nenhum inscrito encontrado</strong><p>As inscrições confirmadas pelo Asaas, os participantes importados da Windfit e as exceções manuais aparecerão aqui com pagamento, categoria, elegibilidade e vínculo com o Ride with GPS.</p></div></div>}
      </section>
    </section>
  </div></main>;
}
