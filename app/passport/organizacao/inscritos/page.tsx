...
export default function RegistrationsPage() {
  ...
  async function load(preferredEvent?: string) {
    ...
    setItems(payload.registrations ?? []); setSummary(payload.summary ?? null); setNumberingReady(payload.numbering_ready !== false); setSequenceRows(payload.sequences ?? []); setForm((current) => ({ ...current, event_id: current.event_id || nextEvent })); if (payload.message) setMessage(payload.message);
  }
