...
type Registration = {
  ...
  imported_at: string | null; last_synced_at: string | null; athlete?: { strava_athlete_id?: number } | null;
};
type Summary = {
  total: number; eligible: number; paid: number; payment_pending: number; refunded: number;
  cancelled: number; linked: number; last_sync: string | null;
};
type FormState = {
...
const emptyForm: FormState = {
  ...
  gender: "", category: "", modality: "gravel_race", country_code: "BR", city: "",
  status: "confirmed", payment_status: "courtesy", external_registration_id: "",
};
...
function parseCsv(text: string) {
  ...
  const aliases: Record<string, string[]> = {
    ...
    bib_number: ["numero", "numero_atleta", "numero_do_atleta", "bib", "bib_number"],
    birth_date: ["nascimento", "data_nascimento", "data_de_nascimento", "birth_date"],
    gender: ["sexo", "genero", "gender"], category: ["categoria", "category"],
    modality: ["modalidade", "produto", "prova", "modality"], country_code: ["pais", "country", "country_code"],
    city: ["cidade", "city"], status: ["status", "situacao", "situacao_inscricao"],
  ...
  return lines.slice(1).map((line) => {
    ...
    return {
      ...
      modality: modalityText.includes("experience") || modalityText.includes("turismo") ? "experience" : "gravel_race",
      country_code: read("country_code") || "BR", city: read("city") || null, status: read("status") || "confirmed",
      registration_code: read("registration_code") || undefined,
      external_registration_id: read("external_registration_id") || null,
      payment_status: read("payment_status") || undefined,
    };
...
function paymentLabel(value: string) {
  return value === "paid" ? "Pago" : value === "pending" ? "Pendente" : value === "refunded" ? "Reembolsado" : value === "cancelled" ? "Cancelado" : "Cortesia";
}

export default function RegistrationsPage() {
  const [events, setEvents] = useState<EventRow[]>([]); const [items, setItems] = useState<Registration[]>([]);
  const [summary, setSummary] = useState<Summary | null>(null); const [eventId, setEventId] = useState("");
  const [search, setSearch] = useState(""); const [statusFilter, setStatusFilter] = useState("all");
  const [paymentFilter, setPaymentFilter] = useState("all"); const [editingId, setEditingId] = useState("");
  const [form, setForm] = useState<FormState>(emptyForm); const [message, setMessage] = useState("");
  const [saving, setSaving] = useState(false); const [moduleReady, setModuleReady] = useState(true);
  ...
  async function load(preferredEvent?: string) {
    ...
    setModuleReady(payload.module_ready !== false); setWindfitReady(payload.windfit_ready !== false);
    setEvents(payload.events ?? []); const nextEvent = selected || payload.events?.[0]?.id || "";
    if (!selected && nextEvent) setEventId(nextEvent); setItems(payload.registrations ?? []); setSummary(payload.summary ?? null);
  ...
  function edit(item: Registration) {
    setEditingId(item.id); setForm({
      event_id: item.event_id, registration_code: item.registration_code, bib_number: item.bib_number ?? "",
      full_name: item.full_name, email: item.email, birth_date: item.birth_date ?? "", gender: item.gender ?? "",
      category: item.category ?? "", modality: item.modality, country_code: item.country_code ?? "", city: item.city ?? "",
  ...
  `}</style><div className="shell">
    <section className="head"><div><p className="kicker">Windfit → Legends Core</p><h1>Inscritos Windfit</h1></div><div><p>A Windfit permanece responsável por inscrição e pagamento. Esta área funciona como espelho operacional para elegibilidade, categoria, número e vínculo com o Strava.</p><div className="sync">Última sincronização: <strong>{formatDateTime(summary?.last_sync)}</strong></div></div></section>
    ...
    <section className="metrics">{[[summary?.total,"registros"],[summary?.eligible,"elegíveis"],[summary?.paid,"pagos"],[summary?.linked,"Strava vinculado"],[summary?.payment_pending,"pagamento pendente"],[summary?.refunded,"reembolsados"]].map(([value,label])=><div className="metric" key={String(label)}><strong>{value??"—"}</strong><span>{label}</span></div>)}</section>
    ...
    <section className="workspace">
      <form className="panel" onSubmit={save}><h2>{editingId?"Editar registro":"Exceção manual"}</h2><p className="panel-intro">Use o cadastro manual somente para cortesia, convidado ou correção administrativa. Inscrições comerciais devem entrar pela importação Windfit.</p><div className="form-grid">
        ...
        <div className="field"><label>ID Windfit</label><input value={form.external_registration_id} onChange={(e)=>setForm({...form,external_registration_id:e.target.value})}/></div>
