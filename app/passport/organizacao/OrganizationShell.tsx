...
function ShellContent({children}:{children:ReactNode}){
  ...
  return <div className="org-shell">
    ...
    `}</style>
    <header className="org-nav"><a href="/passport/organizacao"><img src="/legends-logo-official.png" alt="Legends" /></a><div className="org-nav-right"><div className="org-event"><label>Evento ativo</label><select aria-label="Evento ativo" value={activeEventId} disabled={loading||!events.length} onChange={(event)=>setActiveEventId(event.target.value)}><option value="">{loading?"Carregando eventos...":"Selecione um evento"}</option>{events.map((event)=><option key={event.id} value={event.id}>{event.is_test?"[TESTE] ":""}{event.name}</option>)}</select></div><nav className="org-links"><a href="/passport/organizacao">Resumo</a><a href="/passport/organizacao/eventos">Eventos</a><a href="/passport/organizacao/inscritos">Inscritos</a><a href="/passport/organizacao/revisoes">Revisões</a><a href="/passport/organizacao/validacao">Validar</a><a href="/passport/organizacao/rotas">Rotas</a><a href="/passport/organizacao/checkpoints">Checkpoints</a><a href="/passport/organizacao/rankings">Trechos</a><a href="/passport/organizacao/classificacao">Classificação</a><button onClick={logout}>Sair</button></nav></div></header>
    {children}
  </div>;
}
