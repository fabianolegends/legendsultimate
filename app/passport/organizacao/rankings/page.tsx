...
export default function RankingsPage() {
  ...
  async function load(currentStageId = stageId, currentSegmentId = segmentId) {
    setLoading(true); const query = new URLSearchParams(); if(activeEventId) query.set("eventId",activeEventId); if (currentStageId) query.set("stageId", currentStageId); if (currentSegmentId) query.set("segmentId", currentSegmentId);
  ...
  useEffect(() => { if(!activeEventId){setPayload(null);setStageId("");return;}setSegmentId("");load("", "").then((data) => setStageId(data.stages?.[0]?.id ?? "")).catch((error) => { setStatus(error.message); setLoading(false); }); }, [activeEventId]);
  ...
  `}</style><div className="rankings-shell">
    <p className="eyebrow">Legends Core · Segment Engine</p><h1>Rankings de trechos</h1><p className="intro"><strong style={{color:"#ef8a43"}}>{activeEvent?.name ?? "Selecione um evento"}</strong><br/>Compare o tempo registrado entre dois checkpoints. Os dados oficiais de número, categoria e modalidade vêm do cadastro de inscritos.</p>
