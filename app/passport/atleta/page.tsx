...
type Certificate = { enabled:boolean; available:boolean; template_url:string|null; text_color:string; total_time_s:number|null; total_points:number|null; category_position:number|null; issued_at:string|null };
type Registration = { id:string; event_id:string; bib_number?:string|null; full_name:string; email:string; category?:string|null; modality?:string|null; country_code?:string|null; city?:string|null; status:string; registration_code:string; stage_count?:number; event?:{id:string;slug:string;name:string;starts_on:string;ends_on:string;location?:string|null}|null; certificate?:Certificate };
type Dashboard = {
...
export default function AthletePassportPage() {
  ...
  async function claimRegistration(event:FormEvent){
    ...
    try{
      ...
      setMessage("Evento vinculado ao seu Passport.");setRegistrationCode("");setRegistrationEmail("");setShowClaimForm(false);await loadDashboard(payload.registration?.event_id);
  ...
  async function downloadCertificate(registration:Registration){
    const certificate=registration.certificate;
    if(!certificate?.available||!certificate.template_url)return;
    setCertificateDownloading(true);setMessage("Preparando seu certificado...");
    try{
      ...
      function line(text:string,y:number,size:number,weight=700,maxWidth=.78){
        let pixels=size*unit;context.font=`${weight} ${pixels}px Arial, sans-serif`;
        while(pixels>18*unit&&context.measureText(text).width>canvas.width*maxWidth){pixels-=2*unit;context.font=`${weight} ${pixels}px Arial, sans-serif`;}
        context.fillText(text,canvas.width/2,canvas.height*y);
      }
      const eventName=registration.event?.name??"Evento Legends";
      line(registration.full_name,0.49,72,700,.82);
      line(eventName,0.60,38,600,.74);
      line(registration.category??"Categoria geral",0.67,31,500,.70);
  ...
  `}</style>
    ...
    <div className="athlete-shell">
      ...
      {activeRegistration?.certificate?.enabled?<section className="certificate-card"><div><p className="kicker">Conquista oficial</p><h2>Certificado do evento</h2>{activeRegistration.certificate.available?<p>Resultado publicado: {activeRegistration.certificate.category_position}º lugar em {activeRegistration.category} · {formatDuration(activeRegistration.certificate.total_time_s)}.</p>:<p className="certificate-waiting">O certificado será liberado automaticamente quando todas as etapas e a classificação oficial forem publicadas.</p>}</div>{activeRegistration.certificate.available?<button disabled={certificateDownloading} onClick={()=>void downloadCertificate(activeRegistration)}>{certificateDownloading?"GERANDO...":"BAIXAR CERTIFICADO"}</button>:null}</section>:null}
