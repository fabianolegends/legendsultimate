...
export default function AthletePassportPage() {
  ...
  `}</style><style>{`
    .claim-progress{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:28px}.claim-step{display:flex;gap:12px;align-items:center;border:1px solid #343a32;padding:13px;color:#aeb3ab}.claim-step b{display:grid;place-items:center;width:28px;height:28px;border-radius:50%;background:#2e342d;color:#fff;flex:0 0 auto}.claim-step strong,.claim-step span{display:block}.claim-step span{font-size:12px;margin-top:2px}.claim-step.done{border-color:#3d7955;color:#dce8df}.claim-step.done b{background:#3d7955}.claim-step.current{border-color:#d47b2d;color:#fff}.claim-step.current b{background:#d47b2d}.claim-field{display:grid;gap:7px;min-width:0}.claim-field label{font-size:12px;font-weight:800;color:#d8d4cc}.claim input{min-width:0;font-size:16px}.claim button:disabled{opacity:.65;cursor:wait}.claim-help{grid-column:1/-1;margin:0!important;font-size:12px}
    @media(max-width:850px){.claim-progress{grid-template-columns:1fr}.claim{padding:24px 20px}.claim h2{font-size:clamp(30px,10vw,42px)}.claim input::placeholder{font-size:14px}.claim button{font-size:15px;line-height:1.25}.athlete-actions>span{display:none}}
  `}</style>
    ...
    <div className="athlete-shell">
      <section className="hero"><div><p className="kicker">Portal do atleta</p><h1>Legends Passport</h1><p>Selecione seu evento e depois a etapa para acompanhar homologação, checkpoints e rankings especiais.</p></div><div className="metrics"><div className="metric"><strong>{dashboard?.registration_required?"—":validatedCount}</strong><span>homologadas</span></div><div className="metric"><strong>{dashboard?.registration_required?"—":submissions.length}</strong><span>enviadas</span></div><div className="metric"><strong>{dashboard?.registration_required?"—":visibleStages.length}</strong><span>{dashboard?.registration_required?"após o vínculo":"etapas no evento"}</span></div></div></section>
      {registrations.length?<section className="my-events"><div className="my-events-head"><div><p className="kicker">Minha temporada</p><h2>Meus eventos</h2></div><button className="add-event" onClick={()=>setShowClaimForm(true)}>+ Adicionar evento</button></div><div className="event-tabs">{registrations.map(item=><button key={item.id} className={`event-tab ${item.event_id===selectedEventId?"active":""}`} onClick={()=>chooseEvent(item.event_id)}><strong>{item.event?.name??"Evento Legends"}</strong><span>{item.event?.starts_on?new Date(`${item.event.starts_on}T12:00:00`).toLocaleDateString("pt-BR"):"Data a definir"} · {item.stage_count??0} etapas</span><span>{item.event?.location??"Local a definir"}</span></button>)}</div></section>:null}
      {activeRegistration?<section className="registration-card"><div><p className="kicker">Inscrição confirmada</p><strong>{activeRegistration.full_name}</strong><span>{activeRegistration.category??"Categoria não definida"} · {modality}</span></div><div><span>Número do atleta</span><div className="registration-bib">{activeRegistration.bib_number??"—"}</div></div></section>:null}
      ...
      {!dashboard?.registration_required&&!showClaimForm?<section className="workspace"><aside className="stage-list">{visibleStages.map((item)=><button key={item.id} className={`stage-button ${stageId===item.id?"active":""}`} onClick={()=>setStageId(item.id)}><strong>{item.name}</strong><span>{item.route_label||item.event_name}</span><span>{statusText[dashboard?.submissions.find((sub)=>sub.stage_id===item.id)?.validation?.status]||"Aguardando atividade"}</span></button>)}</aside><section className="stage-panel">
