...
type Dashboard = { athlete:{full_name:string;profile?:string}; stages:Stage[]; submissions:Submission[]; timing_configured?:boolean };
type Result = {
  report:any;
  validation:{id:string;status:string};
  activity:{database_id:string;file_name:string;points_count:number};
  route:{version:number;file_name:string};
  map:{official_points:[number,number][];activity_points:[number,number][];checkpoints:any[]};
...
function TimingPanel({passages,segments,message}:{passages:Passage[];segments:Array<any>;message?:string|null}){
  return <section className="timing-panel"><div className="timing-head"><div><small>CHECKPOINT ENGINE</small><h3>Horários de passagem</h3></div><span>{passages.filter((passage)=>passage.passed!==false&&(passage.passed_at||passage.checkpoint)).length} registros</span></div>
    ...
    {passages.length?<div className="passage-list">{passages.map((passage,index)=>{const checkpoint=passage.checkpoint;const label=passage.label??checkpoint?.label??`CP ${index}`;const elapsed=passage.elapsed_s;return <div className="passage-row" key={`${passage.checkpoint_id??index}-${label}`}><div><strong>{label}</strong><span>{passage.checkpoint_kind==="start"||checkpoint?.checkpoint_kind==="start"?"Largada":passage.checkpoint_kind==="finish"||checkpoint?.checkpoint_kind==="finish"?"Chegada":"Checkpoint"}</span></div><div><strong>{formatPassage(passage.passed_at)}</strong><span>{formatDuration(elapsed)}</span></div></div>;})}</div>:<p className="timing-empty">Nenhuma passagem cronometrada registrada.</p>}
