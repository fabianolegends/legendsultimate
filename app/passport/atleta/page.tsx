...
export default function AthletePassportPage() {
  ...
  `}</style>
    ...
    <div className="athlete-shell">
      ...
      {dashboard?.registration_required?<form className="claim" onSubmit={claimRegistration}><div className="claim-progress" aria-label="Progresso de acesso"><div className="claim-step done"><b>✓</b><div><strong>Ride with GPS conectado</strong><span>Conta autorizada com sucesso</span></div></div><div className="claim-step current"><b>2</b><div><strong>Vincular inscrição</strong><span>Último passo para liberar o Passport</span></div></div></div><p className="kicker">Passo 2 de 2 · Vincular inscrição</p><h2>Libere seu Passport</h2><p>Use o código enviado pela organização. O vínculo é feito uma única vez entre sua inscrição e esta conta Ride with GPS.</p><div className="claim-grid"><div className="claim-field"><label htmlFor="registration-code">Código da inscrição</label><input id="registration-code" name="registration-code" required autoCapitalize="characters" autoComplete="off" spellCheck={false} placeholder="Ex.: LEG-12AB34CD" value={registrationCode} onChange={(event)=>setRegistrationCode(event.target.value.toUpperCase().trimStart())}/></div><div className="claim-field"><label htmlFor="registration-email">E-mail da inscrição (opcional)</label><input id="registration-email" name="email" type="email" inputMode="email" autoComplete="email" placeholder="nome@exemplo.com" value={registrationEmail} onChange={(event)=>setRegistrationEmail(event.target.value)}/></div><p className="claim-help">O e-mail ajuda a confirmar sua identidade quando solicitado pela organização.</p><button disabled={linking}>{linking?"VINCULANDO...":"VINCULAR E LIBERAR ACESSO"}</button></div></form>:null}
