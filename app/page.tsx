  ...
  return (
    <main className="redesign">
      <style>{`
        ...
        .heroMain{display:flex;align-items:center;padding:70px 0}
        ...
        .heroIntro{font-size:20px;line-height:1.55;color:#ddd5c9;max-width:680px}
        .heroCtas{display:flex;gap:20px;align-items:center;margin-top:34px;flex-wrap:wrap}
        ...
        .heroStat{padding:22px 28px;border-right:1px solid var(--line)}
        .heroStat:last-child{border:0}.heroStat strong{display:block;font:700 48px 'Barlow Condensed'}.heroStat span{text-transform:uppercase;letter-spacing:.12em;color:#b8b0a4;font:500 12px 'Barlow Condensed'}
        .manifestoRedesign{background:var(--paper);color:var(--ink);padding:140px 0}
        .manifestoQuote{font:700 clamp(42px,5vw,78px) 'Barlow Condensed';text-transform:uppercase;line-height:.94;max-width:1120px;margin:0 0 54px}.manifestoQuote em{font-style:normal;color:var(--copper)}
        .manifestoGrid{display:grid;grid-template-columns:1fr 1fr;gap:70px;border-top:1px solid #bbb2a5;padding-top:34px}.manifestoGrid p{font-size:18px;line-height:1.85;color:#4d504a;margin:0}
        .factsBand{background:#111411;color:#f1ece3;padding:120px 0}
        .factsGrid{display:grid;grid-template-columns:repeat(4,1fr);border-top:1px solid var(--line);border-bottom:1px solid var(--line)}
        ...
        .previewNotice{position:fixed;right:18px;bottom:18px;background:var(--copper);color:#0b0d0c;padding:10px 14px;z-index:30;font:700 12px 'Barlow Condensed';text-transform:uppercase;letter-spacing:.1em}
        @media(max-width:1000px){.redesign .wide{width:min(100% - 48px,920px)}.redesign .sectionHead,.destinationText,.profileBox{grid-template-columns:1fr}.factsGrid,.safetyGrid,.stageGridNew{grid-template-columns:1fr 1fr}.modeGrid{grid-template-columns:1fr}.includedGrid{grid-template-columns:1fr 1fr}}
        @media(max-width:640px){.redesign .wide{width:calc(100% - 32px)}.heroRedesign{grid-template-rows:82px 1fr auto;min-height:100svh;background-position:68% center}.heroMain{padding:44px 0}.heroCopy h1{font-size:56px}.heroIntro{font-size:16px}.heroStats{grid-template-columns:1fr 1fr}.heroStat:nth-child(2){border-right:0}.heroStat:nth-child(-n+2){border-bottom:1px solid var(--line)}.heroStat strong{font-size:38px}.manifestoRedesign,.factsBand,.modes,.daySection,.routePreview,.destinationStory,.includedSection,.safetySection,.profileSection,.faqSection{padding:90px 0}.manifestoGrid,.factsGrid,.stageGridNew,.safetyGrid,.includedGrid,.cityStrip{grid-template-columns:1fr}.fact,.stageNew,.safetyCard,.cityStrip a{border-right:0;border-bottom:1px solid var(--line)}.manifestoGrid{gap:22px}.modeCard{padding:34px 26px}.timelineItem{grid-template-columns:90px 1fr;gap:18px}.stageNew{min-height:330px}.destinationText{gap:20px}.cityStrip a:last-child{padding-left:0}.includedGrid{background:transparent;border:0}.includedItem{border-bottom:1px solid var(--line)}.profileBox{gap:34px}.previewNotice{font-size:10px;right:10px;bottom:10px}}
      `}</style>

      <div className="previewNotice">Prévia — não publicada</div>
      ...
      <section className="heroRedesign" id="inicio">
        <nav className="nav wide" aria-label="Navegação principal">
          ...
          <div className="navLinks"><a href="#conceito">A prova</a><a href="#modalidades">Modalidades</a><a href="#percurso">Percurso</a><a href="#destino">Destino</a><a href="#faq">FAQ</a></div>
          <a className="navCta" href="#interesse">Lista prioritária</a>
        </nav>
        <div className="heroMain wide">
          <div className="heroCopy">
            <p className="kicker">Serra Gaúcha · Brasil</p>
            <h1>Onde o asfalto termina, a lenda começa.</h1>
            <p className="heroIntro"><strong>Uma Stage Race Premium de Gravel pela Serra Gaúcha.</strong><br />Quatro etapas, quatro cidades e uma travessia criada para quem procura muito mais do que uma prova.</p>
            <div className="heroCtas"><a className="button" href="#conceito">Conheça a Legends <span>→</span></a><a className="secondaryCta" href="#interesse">Entre para a lista prioritária</a></div>
          </div>
        </div>
      ...
      <section className="manifestoRedesign" id="conceito"><div className="wide">
        <p className="kicker">Por que ela existe</p>
        <p className="manifestoQuote">A Legends não foi criada para quem procura apenas uma medalha. Foi criada para quem acredita que a bicicleta é o melhor passaporte para <em>descobrir lugares, pessoas e histórias.</em></p>
        <div className="manifestoGrid"><p>A Legends Ultimate é uma travessia competitiva e turística de quatro dias. Cada etapa leva o participante a uma nova cidade da Serra Gaúcha, conectando esporte, hospitalidade, gastronomia e paisagens que poucos ciclistas do mundo conhecem.</p><p>A classificação importa, mas a jornada importa ainda mais. O atleta recebe suporte para bagagem, hospedagem, recuperação e manutenção, sem perder a autonomia e o desafio que tornam uma verdadeira experiência de gravel inesquecível.</p></div>
      </div></section>

      <section className="factsBand"><div className="wide">
        <div className="sectionHead"><div><p className="kicker">O que torna a Legends diferente</p><h2>Aventura com estrutura.<br /><em>Autonomia com cuidado.</em></h2></div><p>Uma experiência pensada para eliminar a complexidade logística sem retirar do atleta o protagonismo da jornada.</p></div>
        <div className="factsGrid">{differences.map(([n,title,text]) => <div className="fact" key={n}><span>{n}</span><h3>{title}</h3><p>{text}</p></div>)}</div>
      </div></section>

      <section className="modes" id="modalidades"><div className="wide">
        <div className="sectionHead"><div><p className="kicker">Escolha como viver a Legends</p><h2>Competir ou experimentar.<br /><em>A jornada é a mesma.</em></h2></div><p>O participante escolhe o propósito: disputar a classificação no gravel ou realizar a travessia em modo turismo.</p></div>
        <div className="modeGrid"><div className="modeCard dark"><span className="tag">Modalidade competitiva</span><h3>Legends Gravel Race</h3><p>Para ciclistas de gravel ou cyclocross que desejam disputar as quatro etapas com tempo registrado, classificação por pontos, categorias e premiação final.</p><ul><li>Classificação oficial</li><li>Pontuação por etapa</li><li>Categorias por idade</li><li>Troféus ao final da travessia</li></ul></div><div className="modeCard"><span className="tag">Modalidade turismo</span><h3>Legends Experience</h3><p>Para participantes de MTB e E-bike que desejam viver o mesmo percurso e a mesma estrutura, sem pressão por tempo, ranking ou resultado competitivo.</p><ul><li>Sem classificação</li><li>MTB e E-bike permitidas</li><li>Mesma logística e segurança</li><li>Foco em turismo e desafio pessoal</li></ul></div></div>
      </div></section>

      <section className="daySection" id="experiencia"><div className="wide">
        <div className="sectionHead"><div><p className="kicker">Um dia na Legends</p><h2>Pedalar. Recuperar.<br /><em>Descobrir. Recomeçar.</em></h2></div><p>Cada dia tem seu próprio ritmo, mas todos terminam preparando o corpo, a bicicleta e a estratégia para um novo capítulo.</p></div>
        <div className="timeline">{timeline.map(([time,text]) => <div className="timelineItem" key={time+text}><strong>{time}</strong><span>{text}</span></div>)}</div>
      </div></section>

      <section className="routePreview" id="percurso"><div className="wide">
        <div className="sectionHead"><div><p className="kicker">Quatro etapas</p><h2>Uma história contínua.<br /><em>Quatro destinos.</em></h2></div><p>360 quilômetros, aproximadamente 6.000 metros de ascensão e 70% do percurso em estradas de terra.</p></div>
        <div className="stageGridNew">{stages.map(stage => <a className="stageNew" href={stage.href} key={stage.n}><small>{stage.n}</small><img src={stage.route} alt="" /><div><h3>{stage.city}</h3><p>{stage.stats}</p></div></a>)}</div>
      </div></section>

      <section className="destinationStory" id="destino"><div className="wide">
        <div className="destinationText"><div><p className="kicker">O Brasil que poucos conhecem</p><h2>A Serra não é cenário.<br /><em>Ela faz parte da prova.</em></h2></div><div><p>A Serra Gaúcha reúne estradas rurais, araucárias, vales, cânions, pequenas comunidades, gastronomia reconhecida e cidades marcadas pela imigração europeia. Cada etapa revela um novo território e uma nova forma de conhecer o Brasil sobre duas rodas.</p><p>O ciclista estrangeiro virá à Legends para viver, de gravel, uma travessia premium por um Brasil surpreendente que poucos no mundo conhecem.</p></div></div>
        <div className="cityStrip"><a href="https://canela.com.br/" target="_blank"><strong>Canela</strong><span>natureza, arquitetura e gastronomia →</span></a><a href="https://www.saofranciscodepaula.rs.gov.br/portal/turismo" target="_blank"><strong>São Francisco de Paula</strong><span>campos de altitude, araucárias e tradição →</span></a><a href="https://www.gramadoinesquecivel.tur.br/" target="_blank"><strong>Gramado</strong><span>hospitalidade, cultura e experiência premium →</span></a><a href="https://turismo.novapetropolis.rs.gov.br/" target="_blank"><strong>Nova Petrópolis</strong><span>paisagem rural e herança germânica →</span></a></div>
      </div></section>

      <section className="includedSection"><div className="wide">
        <div className="sectionHead"><div><p className="kicker">O que está incluído</p><h2>Você cuida da jornada.<br /><em>Nós cuidamos da estrutura.</em></h2></div><p>O pacote foi desenhado para que o participante concentre energia no percurso, na recuperação e na experiência.</p></div>
        <div className="includedGrid">{included.map(item => <div className="includedItem" key={item}>{item}</div>)}</div>
      </div></section>

      <section className="safetySection"><div className="wide">
        <div className="sectionHead"><div><p className="kicker">Segurança e suporte</p><h2>Aventura exige liberdade.<br /><em>Confiança exige estrutura.</em></h2></div><p>A operação utiliza comunicação, rastreamento e resgate dimensionados para uma travessia de múltiplos dias.</p></div>
        <div className="safetyGrid">{safety.map(([title,text]) => <div className="safetyCard" key={title}><h3>{title}</h3><p>{text}</p></div>)}</div>
      </div></section>
      ...
      <section className="faqSection" id="faq"><div className="wide">
        <div className="sectionHead"><div><p className="kicker">Perguntas frequentes</p><h2>Antes de aceitar o desafio,<br /><em>entenda a jornada.</em></h2></div><p>As informações definitivas de data, valores, hotéis e programação serão publicadas após a confirmação oficial da primeira edição.</p></div>
