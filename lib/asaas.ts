...
function environment() {
  return process.env.ASAAS_ENVIRONMENT?.trim().toLowerCase() === "production" ? "production" : "sandbox";
}
...
function siteUrl() {
  return (process.env.NEXT_PUBLIC_SITE_URL?.trim() || "https://www.legendsbikerace.com.br").replace(/\/+$/, "");
}
...
export async function createAsaasCheckout(input: CheckoutInput, fetcher: typeof fetch = fetch) {
  const maxInstallments = Math.min(21, Math.max(1, Math.round(input.maxInstallments)));
  const expiresMinutes = Math.min(1440, Math.max(10, Math.round(input.expiresMinutes)));
  const callbackUrl = `${siteUrl()}/eventos/${encodeURIComponent(input.eventSlug)}/pagamento`;
  const chargeTypes = maxInstallments > 1 ? ["DETACHED", "INSTALLMENT"] : ["DETACHED"];
  const payload: Record<string, unknown> = {
    ...
    items: [{
      ...
      description: input.modality === "experience" ? "Modalidade Experience" : "Modalidade Gravel Race",
      quantity: 1,
  ...
  if (maxInstallments > 1) payload.installment = { maxInstallmentCount: maxInstallments };
  ...
  const body = await response.json().catch(() => ({})) as AsaasCheckoutResponse;
  if (!response.ok || !body.id) {
    const detail = body.errors?.map((item) => item.description || item.code).filter(Boolean).join(" ");
    throw new Error(detail || `O Asaas recusou a criação do checkout (HTTP ${response.status}).`);
  }
  return {
    id: body.id,
...
export async function cancelAsaasCheckout(checkoutId: string, fetcher: typeof fetch = fetch) {
  const response = await fetcher(`${asaasApiBaseUrl()}/checkouts/${encodeURIComponent(checkoutId)}/cancel`, {
    method: "POST",
    headers: {
      accept: "application/json",
      "content-type": "application/json",
  ...
  if (!response.ok && response.status !== 404) {
    const body = await response.json().catch(() => ({})) as AsaasCheckoutResponse;
    const detail = body.errors?.map((item) => item.description || item.code).filter(Boolean).join(" ");
    throw new Error(detail || `Não foi possível cancelar o checkout Asaas (HTTP ${response.status}).`);
  }
}
